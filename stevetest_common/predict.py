...
from stevetest_common import feature, const, dto, rules_new
import copy
import logging
...
def predict(model_lv1, input_str, top=1, product_id=None, item_id=None, garbage_filter=True):
    ...
    pred_lv1s.sort(key=lambda x: x.get_final_score(), reverse=True)
    return pred_lv1s[:top]
